...
tang.init().block([
    ...
    var types = {
        ...
        declareModel = function(options, name) {
            ...
            _.each(options, function(i, option) {
                if (_.util.isArr(option) && option.length > 0) {
                    if (option.length > 1) {
                        if (option.length > 2) {
                            if (_.util.isNumeric(option[2])) {
                                var length = parseInt(option[2]) || 0;
                                if (option.length > 3) {
                                    if (_.util.isArr(option[3])) {
                                        ...
                                        if (option.length > 4 && _.util.isArr(option[4]) && option[4].length > 0) {
                                            ...
                                            if (option.length > 5 && _.util.isArr(option[5]) && option[5].length >= option[4].length) {
                                                var select = option[5];
                                            } else {
                                                var select = range;
                                            }
                            ...
                            } else if (_.util.isArr(option[2])) {
                                var length = 0;
                                if (option[2].length > 0) {
                                    var defaultValue = option[2][0],
                                        range = option[2];
                                    if (option.length > 3 && _.util.isArr(option[3]) && option[3].length >= option[2].length) {
                                        var select = option[3];
                                    } else {
                                        var select = range;
                                    }
                                ...
                                var length = 0;
                                if (option.length > 3 && _.util.isArr(option[3]) && option[3].length > 0) {
                                    ...
                                    if (option.length > 4 && _.util.isArr(option[4]) && option[4].length >= option[3].length) {
                                        var select = option[4];
                                    } else {
                                        var select = range;
                                    }
    ...
    declare('form.VisualJOSN.ClassObject', _.form.VisualJOSN.Object, {
        ...
        _init: function(elem, $model, textarea) {
            if (_.util.isStr($model) && /\w+/.test($model)) {
                if (models[$model]) {
                    this.model = models[$model];
                } else {
                    return _.error('class [' + classname + '] not declared.');
                }
            } else if (_.util.isArr($model)) {
    ...
    _.extend(_.form.VisualJOSN.ClassObject, true, {
        declare: function(classname, options) {
            if (_.util.isStr(classname) && /\w+/.test(classname) && _.util.isArr(options)) {
                var _i = 0,
