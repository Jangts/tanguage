 ...
 *
 * Datetime: Sat, 04 Aug 2018 11:26:31 GMT
 */
;
// tang.config({});
tang.init().block([
    ...
    pandora.declareClass('app.Slider', {
        data: null,
        theme: 'default',
        actorsType: 'image',
        speed: 5000,
        ...
        _init: function (elem, preset, options) {
            this.HTMLElement = bool.isStr(elem) ? pandora.dom.selector.byId(elem): elem;
            if (bool.isEl(this.HTMLElement)) {
                options = options || {}
                if (bool.isStr(preset) && pandora.app.Slider.presets[preset]) {
                    pandora.extend(options, pandora.app.Slider.presets[preset]);
                }
                else if (bool.isObj(preset)) {
                    pandora.extend(options, preset);
                }
                else {
                    pandora.extend(options, pandora.app.Slider.presets.slide);
                }
                pandora.extend(this, true, options);
                if (this.HTMLElement && this.layout) {
                    var that = this;
                    $(this.HTMLElement).addClass(preset);
                    this.data && this.bluider && this.bluider();
                    this.stages = $('.stage', this.HTMLElement).addClass(bool.isArr(this.stageTheme) ? function (i) {
                        return that.theme[i];
                    } : this.stageTheme);
                    this.troupe = $('.troupe', this.HTMLElement).get(0);
                    this.actors = $('.actor', this.troupe);
                    this.panelTheme = this.panelTheme || this.stageTheme;
                    this.panel = $('.panel', this.HTMLElement).addClass(bool.isArr(this.panelTheme) ? function (i) {
                        return that.panelTheme[i];
                    } : this.panelTheme);
                    this.anchors = $('.slider-anchor', this.HTMLElement);
        ...
        bind: function () {
            ...
            this.pauseOnHover && $(this.HTMLElement)
    ...
    pandora.extend(pandora.app.Slider, {
        presets: {},
        extend: function () {
            ...
            pandora.each(args, function (_index, preset) {
                if (bool.isStr(preset) && pandora.app.Slider.presets[preset]) {
                    _.extend(base, pandora.app.Slider.presets[preset]);
                }
            ...
            pandora.each(presets, function (name, preset) {
                pandora.app.Slider.presets[name] = _.extend( {}, base, preset);
            }, this);
        }
    });
    pandora.app.Slider.extend({
        name: 'slide',
        easing: "linear",
        bluider: function () {},
        layout: function () {
            ...
            if (this.renderPanel) {
                $('.panel', this.HTMLElement).each(function () {
