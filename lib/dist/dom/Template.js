...
tang.init().block([
    ...
    var filters = {repeat: function (str, times) {
    ...
    var compile = function (source) {
        ...
        pandora.each(filters, function (filter, handler) {
            ...
            if (matches) {
                pandora.each(matches, function (i, match) {
                    source = source.replace(pattern, "<%echo(filters." + filter + "(this.data.$1,$2))%>");
                }, this);
            };
        }, this);
        pandora.each(order, function (i, group) {
            pandora.each(compilers[group], function (j, type) {
                var matches = source.match(type.pattern);
                if (matches) {
                    pandora.each(matches, function (k, match) {
