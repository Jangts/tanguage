...
tang.init().block([
    ...
    function selector (selector, context) {
        ...
        var result = [];
        if (onlyIdExpr.test(selector_1)) {
            selector_1 = selector_1.replace(/#/, "");
            var elem = doc.getElementById(selector_1);
            if (elem) {
        ...
        if (onlyNameExpr.test(selector_1)) {
            selector_1 = selector_1.replace(/@/, "");
            return context.getElementsByName(selector_1);
        }
        if (doc.querySelectorAll) {
            context = context.querySelectorAll ? context : document;
            if (selector_1.match(/\[[\w-]+=("|')[^\1]*\1[^\]]*\1\]/g)) {
                return [];
            }
            selector_1 = selector_1.replace(/\[[\w-]+=[^'"=\]]+\]/g, function (attr) {
                return attr.replace("=", '="').replace("]", '"]');
            }).replace(/\[+/, '[').replace(/\]+/, ']');
            result = context.querySelectorAll(selector_1);
        }
        else {
            pandora.each(selector_1.split(','), function (_index, s) {
    ...
    function filters (selector, context) {
        var _arguments = arguments;
        var result = [];
        var splits = selector_11.match(gtExpr);
        if (splits) {
            return childFilters(splits, context);
        }
        splits = selector_11.split(/\s+/);
    ...
    function find (selector, context) {
        var result = [];
        var className = selector_18.match(classExpr);
        var id = !className && selector_18.match(idExpr);
        var attr = !id && selector_18.match(attrExpr);
        if (id) {
            ...
            result = byTagName(selector_18, context);
        }
    ...
    function byAttrsObject (Attrs, context) {
        ...
        if (Attrs.className) {
            ...
            else if (pandora.util.gettype(Attrs.className) == 'Array') {
    ...
    pandora.extend(pandora, {
        byId: byId,
        byCn: byClass,
        selector: pandora.dom.sizzle || selector
    });
