...
tang.init().block([
    ...
    declare('see.Tabs.TabViews', {
        ...
        build: function(options) {
            this.tabs = {};
            options = options || {};
            if (_.util.isStr(options.starttab)) {
                this.startTabName = options.starttab.toUpperCase();
            }
            if (_.util.isFn(options.start)) {
                this.start = options.start;
            }
            if (_.util.isFn(options.onbeforeunload)) {
                this.onbeforeunload = options.onbeforeunload;
            }
            if (_.util.isFn(options.onbeforewrite)) {
                this.onbeforewrite = options.onbeforewrite;
            }
            if (_.util.isFn(options.onbeforecut)) {
                this.onbeforecut = options.onbeforecut;
            }
            if (_.util.isFn(options.onaftercut)) {
                this.onaftercut = options.onaftercut;
            }

            if (_.util.isFn(options.onafterdestroy)) {
