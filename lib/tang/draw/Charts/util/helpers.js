...
tang.init().block([
    ...
    var aliasPixel = function(pixelWidth) {
        ...
        calculateCircleStyle = function(options, width, height) {
            ...
            if (options.radius) {
                if (_.util.isPercent(options.radius)) {
                    radius = maxRadius * parseInt(options.radius) / 100
                } else if (_.util.isNumeric(options.radius)) {
                    radius = parseFloat(options.radius);
                }
            }

            if (_.util.isArr(options.center) && options.center.length === 2) {
                if (_.util.isPercent(options.center[0])) {
                    x = width * parseInt(options.center[0]) / 100
                } else if (_.util.isNumeric(options.center[0])) {
                    x = parseFloat(options.center[0]);
                }
                if (_.util.isPercent(options.center[1])) {
                    y = height * parseInt(options.center[1]) / 100
                } else if (_.util.isNumeric(options.center[1])) {
        ...
        calculateDoughnutStyle = function(options, width, height) {
            ...
            innerRadius = 0,
                ...
                x = width / 2,
                y = height / 2;
            if (_.util.isArr(options.radius)) {
                if (options.radius[0]) {
                    if (_.util.isPercent(options.radius[0])) {
                        innerRadius = maxRadius * parseInt(options.radius[0]) / 100
                    } else if (_.util.isNumeric(options.radius[0])) {
                        innerRadius = parseFloat(options.radius[0]);
                    }
                }
                if (options.radius[1]) {
                    if (_.util.isPercent(options.radius[1])) {
                        outerRadius = maxRadius * parseInt(options.radius[1]) / 100
                    } else if (_.util.isNumeric(options.radius[1])) {
        ...
        calculateRectangleStyle = function(options, width, height) {
            var width,
                ...
                bottom,
                left;
            if (_.util.isNumeric(options.top)) {
                top = parseFloat(options.top);
            } else if (_.util.isPercent(options.top)) {
                top = height * parseFloat(options.top) / 100;
            } else {
                top = 0;
            }

            if (_.util.isNumeric(options.right)) {
                right = parseFloat(options.right);
            } else if (_.util.isPercent(options.right)) {
                right = width * parseFloat(options.right) / 100;
            } else {
                right = 0;
            }

            if (_.util.isNumeric(options.width)) {
                width = parseFloat(options.width);
                left = width - right - width;
            } else if (_.util.isPercent(options.width)) {
                width = width * parseFloat(options.width) / 100;
                left = width - right - width;
            } else {
                if (_.util.isNumeric(options.left)) {
                    left = parseFloat(options.left);
                } else if (_.util.isPercent(options.left)) {
            ...
            if (_.util.isNumeric(options.height)) {
                height = parseFloat(options.height);
            } else if (_.util.isPercent(options.height)) {
                height = height * parseFloat(options.height) / 100;
            } else {
                if (_.util.isNumeric(options.bottom)) {
                    bottom = parseFloat(options.bottom);
                } else if (_.util.isPercent(options.bottom)) {
    ...
    _.extend(_.draw.Charts.util.helpers, {
        aliasPixel,
        animationLoop,
