    ...
    declare('draw.Charts', {
        ...
        _init: function(elem, theme, options) {
            if (_.util.isElement(elem)) {
                ...
                if (theme && _.draw.Charts.theme[theme]) {
                    this.options = _.obj.deepMerge(_.draw.Charts.util.defaults, _.draw.Charts.theme[theme]);
                } else {
                    this.options = _.copy(_.draw.Charts.util.defaults);
                }
        ...
        setOption: function(options) {
            ...
            _.draw.Charts.util.events.bindHover(this);
            if (options.series) {
        ...
        clear: function() {
            _.each(this.layers, function(zIndex, canvas) {
                var ctx = canvas.getContext("2d"),
                    width = ctx.canvas.width,
                    height = ctx.canvas.height;
                _.draw.canvas.clear(ctx, width, height);
            });
            return this;
        },
        stop: function() {
        ...
        resize: function() {
            this.stop();
            this.clear();
            var newWidth = _.draw.Charts.util.helpers.getMaximumWidth(this.Element),
                newHeight = this.options.maintainAspectRatio ? newWidth / this.ratio : _.draw.Charts.helpers.getMaximumHeight(this.Element);
            this.width = newWidth;
